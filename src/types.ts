...
export interface ActionDefinition {
  ...
  /**
   * Действие не принимает цель и работает только на автора (напр. silly).
   * Слеш-команда регистрируется без опции цели, упоминания игнорируются.
   */
  noTarget?: boolean;

  /**
   * Гифки задавать здесь НЕ нужно. Они хранятся в config.json (секция gifs.<name>,
   * ключ = name) и читаются «вживую» при каждом вызове через lib/gifStore.ts,
   * поэтому их можно менять без перезапуска бота. Поле оставлено опциональным
