...
**Перезапуск не нужен.** Гифки читаются из `config.json` «вживую»: бот замечает
изменение файла и подхватывает новые ссылки при следующем вызове команды.
Достаточно сохранить `config.json` (в Docker он смонтирован как volume, так что
правки на хосте видны контейнеру сразу).

> Перезапуск/повторная регистрация нужны только при изменении самих команд
> (новый файл в `src/actions`, переименование, смена шаблонов) — это код, а не гифки.
